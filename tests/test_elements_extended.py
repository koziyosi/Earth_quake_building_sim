...
class TestAsymmetricBilinear:
    def test_elastic_range(self):
        ...
        hyst.set_trial_displacement(0.05)  # Within elastic range

        assert abs(hyst.trial_force - 50) < 1e-6  # k0 * disp = 1000 * 0.05
        assert hyst.trial_tangent == 1000

    def test_positive_yield(self):
        """Test yielding in positive direction."""
        hyst = AsymmetricBilinear(k0=1000, fy_pos=100, fy_neg=80, r=0.1)

        hyst.set_trial_displacement(0.2)  # Beyond yield

        assert hyst.trial_force > 100
        assert hyst.trial_tangent == 100  # k0 * r

    def test_negative_yield(self):
        """Test yielding in negative direction (buckling)."""
        hyst = AsymmetricBilinear(k0=1000, fy_pos=100, fy_neg=80, r=0.1)

        hyst.set_trial_displacement(-0.2)  # Beyond yield in compression

        assert hyst.trial_force < -80
        assert hyst.trial_tangent == 100
